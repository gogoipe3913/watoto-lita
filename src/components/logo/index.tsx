"use client";

import classNames from "classnames";
import React from "react";
import style from "./style.module.scss";

type LogoProps = {
  isColored?: boolean;
  className?: string;
};

const Logo: React.FC<LogoProps> = ({ isColored = false, className = "" }) => (
  <svg
    id="_レイヤー_2"
    data-name="レイヤー 2"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 60.45 176.55"
    className={classNames(style.Logo, className)}
  >
    <g id="_レイヤー_1-2" data-name="レイヤー 1">
      <g>
        <path
          className={classNames(
            style.Logo__path,
            isColored ? style["Logo__path--colored"] : ""
          )}
          d="M19.84,28.82c11.15-7.96,17.36-14.42,31.64-8.39,7.93,3.35,11.17,11.66,7.39,19.38-1.3,2.67-8.5,8.29-11.2,8.94-1.71.41-5.07.49-5.69-1.55-.97-3.21,5.15-5.39,6.94-7.03,5.17-4.72,8.58-14.06-.09-17.09-8.21-2.87-16.14-1.01-23.33,3.38-2.36,1.44-4.6,3.43-4.72,6.45-.05,1.28,1.29,13.63,1.57,14.29.3.71,1.18.74,1.48,1.41,3.04,6.83-5.28,4.94-5.36.99-.02-.8,1.31-1.27,1.41-3.26.16-2.88-1.53-6.83-1.5-9.85-2.78,3.44-6.55,5.11-10.88,2.97-2.77-1.37-2.72-3.78-4.06-6.02s-4.89-4.24-2.83-7.31c1.05-1.57,7.99-2.49,9.87-1.69,5.61,2.38-.92,7.89,1.42,11.57.57.79,1.59-.41,2.12-.78,5.82-4.05,2.99-9.43,3.39-15.81.25-4.05.72-12.15,1.49-15.79,1.24-5.87,7.01-3.74,6.69-.08-.11,1.24-2.38,4.09-2.97,5.72-2.17,5.99-3.99,13.2-2.78,19.53Z"
        />
        <path
          className={classNames(
            style.Logo__path,
            isColored ? style["Logo__path--colored"] : ""
          )}
          d="M29.57,93.66c2.54-.51,4.72-2.18,6.15,1.21-5.44,3.55-5.52,10.49-3.39,16.33l-1.57,1.05c-5.45-4.52-3.27-11.09-3.8-16.83-.32-3.5-1.93-7.86-2.22-11.54,9.39-4.87,3.69,8.5,4.83,9.78Z"
        />
        <path
          className={classNames(
            style.Logo__path,
            isColored ? style["Logo__path--colored"] : ""
          )}
          d="M26.9,141.01c3.4,2.77,5.52,11.82,5.44,16.16l1.72-1.96c4.77,1.52.82,3.69,0,5.36-1.77,3.54-1.33,11.9-.28,15.78-7.43,2.11-2.34-13.27-2.4-17.26-2.48.41-2.47-6.83-3.49-9.7-.32-.89-5.08-9.17-3.45-9.5.77.19,1.88.64,2.48,1.12Z"
        />
      </g>
    </g>
  </svg>
);

export default Logo;
